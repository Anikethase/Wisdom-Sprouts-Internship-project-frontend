...
export class LoginComponent implements OnInit {
  ...
  loginClicked(form:NgForm)
  {
    ...
    this.http.post('http://localhost:3010/api/signIn',logIn).subscribe(responseData => {
  ...
  private handleAuthentication(email: string, userId: string, full_name: string,) 
  {
   
    const user = new Userlogin(email, userId, full_name);
    this.user.next(user);

    localStorage.setItem('userData', JSON.stringify(user)); // option 1
    localStorage.setItem('user_email', email);  // option 2
    localStorage.setItem('user_name', full_name);  // option 3
    localStorage.setItem('user_id', userId);  // otpion 4
 
    sessionStorage.setItem('user_name', full_name);
  }
